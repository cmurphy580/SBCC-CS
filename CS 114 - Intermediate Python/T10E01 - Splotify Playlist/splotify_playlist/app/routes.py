...
def login():
    ...
    return render_template('login.html', title='Splotify', form=form, user_data=user_data, users=users, password=password)
...
def set_playlist(user_id):
    ...
    if request.method == 'POST':
        ...
        if type(remove_song) == int and remove_song != 0:
            ...
            user_data["playlist"] = list(filter(lambda v: v["id"] != remove_song, user_data["playlist"]))
            print("removed!")
            ...
            order_changes = [(i, int(sort_request[i])) for i in range(len(sort_request)) if sort_request[i] != "sort"]
    ...
    return render_template('playlist.html', title='Your Splotify Playlist', user_data=user_data, songs=songs, playlist=user_data["playlist"])
